...
# Word-level OCR analysis (analyzer/word_level_analyzer.py) is an
# additive, optional layer on top of this file's existing global
# caption-style detection. It is never required: if it can't be
# imported (e.g. pytesseract missing) analyze_video() below still
# returns the same global-only profile it always has.
try:
    from analyzer.word_level_analyzer import analyze_word_styles
except ImportError:
    try:
        from word_level_analyzer import analyze_word_styles
    except ImportError:
        analyze_word_styles = None
...
def analyze_video(
    ...
    # --- Word-level analysis (additive; never blocks the global profile) ---
    if analyze_word_styles is not None:
        try:
            word_result = analyze_word_styles(samples, average_region, fps, mode)
            ...
            word_result = {
                ...
                "word_analysis": {
                    ...
                    "confidence": 0.0,
                    "word_count": 0,
                    "message": f"word-level analysis raised: {exc}",
                },
            }
    else:
        word_result = {
            ...
            "word_analysis": {
                ...
                "confidence": 0.0,
                "word_count": 0,
                "message": "word-level analyzer module unavailable",
            },
        }

    if word_result.get("word_detections"):
        style_profile["word_detections"] = word_result["word_detections"]
    if word_result.get("word_styles"):
        style_profile["caption"]["word_styles"] = word_result["word_styles"]
    style_profile["word_analysis"] = word_result["word_analysis"]
